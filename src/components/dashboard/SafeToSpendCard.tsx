    ...
    <Card className="p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 truncate text-3xl font-black tracking-normal text-foreground">{formatCurrency(summary.spendingPower.safeToSpendToday)}</h2>
...
function SafeMetric({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) {
  return (
    <div className="flex min-w-0 items-center justify-between gap-3 rounded-2xl bg-teal-50/70 px-3 py-3">
      ...
      <span className="min-w-0 max-w-[8rem] shrink-0 truncate text-right text-sm font-black text-foreground">{value}</span>
    </div>
  );
}
