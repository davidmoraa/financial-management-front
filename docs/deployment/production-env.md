...
Money Flux frontend is a Vite app. Production builds use Vite's `production` mode, so `npm run build` and `npm run build:production` load `.env.production` unless Hostinger environment variables override the same keys.
